...
var pipeline = [];
var logger, hook, processState, persist, checkForStarving, wakeUp, dispose;
...
persist = function (info) {
	cachemap.set(info.key, info.data);
};
...
processState = function (id, state) {
	switch (state) {
		...
		case States.DATAFETCH:
		case States.COMPILED:
		case States.DUMPED:
		case States.PERSISTED:
		case States.CLEANED:
		case States.BULKED:
		case States.VALIDATED:
			logger.info(id, " is on state ", state);
			break;
		...
		case States.FINISHED:
			logger.info(id, " FSM flow has been completed! Ending process...");
			break;
...
exports.onJobUpdate = function (id, event, args, clear) {
	switch (event) {
		...
		// Job persist info request
		case CustomEvents.ON_JOB_PERSIST:
			// logging
			logger.info(id, " asked to persist data.");
			logger.debug(args);
		...
		// Job has been disconencted
		case CustomEvents.ON_JOB_DISCONNECT:
			logger.warn(id, " has been disconnected!");			
			logger.debug(args);
			break;

		// Job has been closed
		case CustomEvents.ON_JOB_CLOSE:
			logger.warn(id, " has been killed with exit code ", args.code);
